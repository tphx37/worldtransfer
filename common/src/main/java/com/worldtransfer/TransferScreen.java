...
public class TransferScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.literal("Advanced settings"),
                button -> TransferClientPlatform.current().openScreen(Minecraft.getInstance(),
                    new TransferSettingsScreen(this, settings)))
            .bounds(this.width / 2 - 120, bottom, 240, 20)
            .build());

        Button historyButton = Button.builder(Component.literal("Ownership history"),
                button -> TransferClientPlatform.current().openScreen(Minecraft.getInstance(),
                    new TransferHistoryScreen(this)))
        ...
        Button incomingButton = Button.builder(Component.literal(hasIncoming
                ...
                button -> TransferClientPlatform.current().openScreen(Minecraft.getInstance(),
                    new IncomingTransferScreen(this)))
    ...
    private void run(String arguments) {
        Minecraft client = Minecraft.getInstance();
        TransferClientPlatform.current().sendCommand(client, "worldtransfer " + arguments);
        TransferClientPlatform.current().openScreen(client, parent);
    }
